...
def _load_task_text(traj_dir: str) -> str:
...
def convert_one_traj(
    ...
    *,
    ...
    task_text = _load_task_text(traj_dir)
    if require_text and (not isinstance(task_text, str) or task_text.strip() == ""):
        print(f"[convert_one_traj] skip (no text): {traj_dir}")
        return output_root
    ...
    # Write all frames in this trajectory
    any_frames = False
    for t in range(T):
        ...
        frame = {
            **frame_imgs,
            ...
            "observation.state": fs[t].astype(np.float32),
...
def convert_root(
    ...
    *,
    ...
    for epi_idx, traj_dir in enumerate(trajs, start=start_idx):
        ...
        task_text = _load_task_text(traj_dir)
        if require_text and (not isinstance(task_text, str) or task_text.strip() == ""):
            print(f"[convert_root] skip traj (no text): {traj_dir}")
            continue
...
def main():
    parser = argparse.ArgumentParser("Convert raw Bridge data to a LeRobot dataset (no atomic)")
    ...
    parser.add_argument("--require_text", action="store_true", default=True, help="Skip trajectories without text (lang.txt)")
